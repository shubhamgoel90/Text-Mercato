...
# Creating the KMeans Algo
class K_means:

  def __init__(self, k=6, size=False, resample=32):
    self.k = k
    self.cluster = []
    self.data = []
    self.end = []
  ...
  # Rearranging the Clusters --->  Grouping of images based on their distance.
...
# To pass the argument such as how many clusters, folder (pics) Path, etc.
...
# To Work with all types of image format
...
k = K_means(args["kmeans"],args["resample"])
...
for i in range(k.k) :
	try :
	  os.makedirs(folder+"Product "+str(i+1).zfill(nfolders))
	except :
	  print("Folder already exists")
action = shutil.copy

# Instead of copying, moving the images into new folders
if args["move"] :
	action = shutil.move

for i in range(len(k.cluster)):
	action(k.end[i], folder+"/"+"Product "+str(k.cluster[i]+1).zfill(nfolders)+"/")
